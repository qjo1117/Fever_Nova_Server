...
void MainManager::Run() // 서버 실행
{
	while (1) {
		...
		OVERLAPPEDEX* l_acceptOverlappedEX;	// accpet 확장 구조체 (accpet 처리 동안 임시)
		l_clientSock = m_listenSock.Accept(); // Accpet 대기 (동기)
		if (l_clientSock == INVALID_SOCKET)
		{
			continue;// 소켓이 없으면 다시 accpet 대기
		}
		l_acceptOverlappedEX = new OVERLAPPEDEX(); // 구조체 생성
		l_acceptOverlappedEX->operation = E_OPERATION::ACCEPT;
...
bool MainManager::RecvProcess(void* _session, int _completeByte) // Recv완료시 작업
{
	...
	// 매니저 프로세스 변경...
	//LoginManager::GetInstance()->Function(session);
