...
Socket::Socket()
{
	ZeroMemory(&m_sendOverlap, sizeof(OVERLAPPEDEX));
	ZeroMemory(&m_recvOverlap, sizeof(OVERLAPPEDEX));
	ZeroMemory(&m_socket, sizeof(SOCKET));
	ZeroMemory(&m_address, sizeof(SOCKADDR_IN));

	m_sendOverlap.session = this;
	m_sendOverlap.operation = E_OPERATION::SEND;

	m_recvOverlap.session = this;
	m_recvOverlap.operation = E_OPERATION::RECV;
}
...
OVERLAPPEDEX* Socket::GetRecvOverlapped()
{
	return &m_recvOverlap;
}

OVERLAPPEDEX* Socket::GetSendOverlapped()
{
